...
def mapper(csv_file_path, read_line=0):
    ...
    if credit_col and debit_col:
        ...
        df['amount_temp'] = pd.to_numeric(df[credit_col], errors='coerce')
        df.drop(columns=[credit_col], inplace=True)

    elif debit_col:
        df['amount_temp'] = pd.to_numeric(df[debit_col], errors='coerce')
        df.drop(columns=[debit_col], inplace=True)
    elif amount_col:
        df['amount_temp'] = pd.to_numeric(df[amount_col], errors='coerce')
    ...
    # print(f"df_reordered.columns: {df_reordered.columns}")
    # print(f"df_reordered{csv_file_path}: {df_reordered.head(5)}")
    return df_reordered
